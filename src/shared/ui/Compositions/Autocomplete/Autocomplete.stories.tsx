...
import { Autocomplete } from "@/shared/ui/Compositions/Autocomplete/Autocomplete"
import { Select } from "@/shared/ui/Compositions/Select"
...
export const Demo = {
    render: () => {
        const [value, setValue] = useState<null | SelectOption>(options[0])
        const [inputValue, setInputValue] = useState<string>("")

        return (
            <div className="flex flex-col gap-5 items-center pb-20">
                <div>Selected: {value?.label ?? "null"}</div>
                <div>Input Value: {inputValue ?? "null"}</div>
                <Autocomplete
                    options={options}
                    value={value}
                ...
                <Button className="w-full" onClick={() => setInputValue("asd")}>
