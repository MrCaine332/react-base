...
import { LinkedAbortController } from "@/shared/utils/LinkedAbortController"
import {
...
import { AnyQueryClient, MobxQueryClientHooks } from "./mobx-query-client.types"

export class MobxInfiniteQuery<TData, TError = DefaultError, TQueryKey extends QueryKey = any, TPageParam = unknown>
    implements Disposable
{
    protected abortController: AbortController
    protected queryClient: AnyQueryClient

    protected _result: InfiniteQueryObserverResult<InfiniteData<TData, TPageParam>, TError>
    options: MobxInfiniteQueryOptions<TData, TError, TQueryKey, TPageParam>
    queryObserver: InfiniteQueryObserver<TData, TError, InfiniteData<TData, TPageParam>, TData, TQueryKey, TPageParam>
     ...
     */
    private holdedEnabledOption: MobxInfiniteQueryOptions<TData, TError, TQueryKey, TPageParam>["enabled"]
    private _observerSubscription?: VoidFunction
    private hooks?: MobxQueryClientHooks

    constructor(protected config: MobxInfiniteQueryConfig<TData, TError, TQueryKey, TPageParam>) {
        const { queryClient, queryKey: queryKeyOrDynamicQueryKey, options: getDynamicOptions, ...restOptions } = config
        ...
        this.hooks = "hooks" in this.queryClient ? this.queryClient.hooks : undefined

        if (queryClient instanceof MobxQueryClient && config.enableOnDemand == null) {
            this.isEnabledOnResultDemand = queryClient.queryFeatures.enableOnDemand ?? false
        }
        ...
        if (typeof queryKeyOrDynamicQueryKey === "function") {
            ...
            this.options.queryKey = queryKeyOrDynamicQueryKey ?? this.options.queryKey ?? []
        }

        // Tracking props visit should be done in MobX, by default.
        this.options.notifyOnChangeProps =
            restOptions.notifyOnChangeProps ?? queryClient.getDefaultOptions().queries?.notifyOnChangeProps ?? "all"
        ...
        this._observerSubscription = this.queryObserver.subscribe(this.updateResult)
    ...
    protected createQueryHash(queryKey: any, options: MobxInfiniteQueryOptions<TData, TError, TQueryKey, TPageParam>) {
    ...
    setData(
        ...
        this.queryClient.setQueryData<InfiniteData<TData, TPageParam>>(this.options.queryKey, updater, options)
    }
    ...
    private processOptions = (options: MobxInfiniteQueryOptions<TData, TError, TQueryKey, TPageParam>) => {
        ...
        if (this.isEnabledOnResultDemand) {
            ...
            if (this.isResultRequsted) {
                if (this.isEnableHolded) {
                    options.enabled =
                        this.holdedEnabledOption === this.enableHolder ? undefined : this.holdedEnabledOption
     ...
     */
    private updateResult(nextResult: InfiniteQueryObserverResult<InfiniteData<TData, TPageParam>, TError>) {
    ...
    onDone(onDoneCallback: (data: InfiniteData<TData, TPageParam>, payload: void) => void): void {
        reaction(
            () => {
    ...
    protected handleAbort = () => {
        ...
        let isNeedToReset = this.config.resetOnDestroy || this.config.resetOnDispose

        if (this.queryClient instanceof MobxQueryClient && !isNeedToReset) {
            isNeedToReset =
                this.queryClient.queryFeatures.resetOnDestroy || this.queryClient.queryFeatures.resetOnDispose
        }
