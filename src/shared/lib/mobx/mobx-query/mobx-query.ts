...
export class MobxQuery<TData, TError = DefaultError, TQueryKey extends QueryKey = any> implements Disposable {
     ...
     */
    private holdedEnabledOption: MobxQueryOptions<TData, TError, TQueryKey>["enabled"]
    ...
    constructor(queryClient: AnyQueryClient, config: () => QueryOptionsParams<TData, TError, TQueryKey>)

    constructor(...args: any[]) {
        const [queryClient, config]: [AnyQueryClient, QueryOptionsParams<TData, TError, TQueryKey>] =
            args.length === 2 ? [args[0], args[1]()] : [args[0].queryClient, args[0]]
        const { queryKey: queryKeyOrDynamicQueryKey, options: getDynamicOptions, ...restOptions } = config
        this.config = {
            ...config,
            queryClient,
        }
        ...
        this.queryObserver = new QueryObserver(queryClient as QueryClient, this.options)
    ...
    protected createQueryHash(queryKey: any, options: MobxQueryOptions<TData, TError, TQueryKey>) {
    ...
    setData(updater: Updater<NoInfer<TData> | undefined, NoInfer<TData> | undefined>, options?: SetDataOptions) {
        return this.queryClient.setQueryData<TData>(this.options.queryKey, updater, options)
    }
    ...
    private processOptions = (options: MobxQueryOptions<TData, TError, TQueryKey>) => {
    ...
    async start({ cancelRefetch, ...params }: MobxQueryStartParams<TData, TError, TQueryKey> = {}) {
        this.update({ ...params })
