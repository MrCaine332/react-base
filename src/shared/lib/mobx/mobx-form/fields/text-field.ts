 ...
 */
export class TextField<T> implements FieldWithValue<T>, TouchableField, CloneableField<TextField<T>>, FieldWithError {
    isTouched = false
    isDirty = false
    ...
    onChange(value: T) {
        this.value = this.options?.beforeChange ? this.options.beforeChange(value) : value
        this.isDirty = true
